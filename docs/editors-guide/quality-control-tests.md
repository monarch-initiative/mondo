...
SELECT DISTINCT ?entity ?property ?value WHERE
{
  VALUES ?value { 
    ...
    <http://purl.obolibrary.org/obo/mondo#doid_rare>
...
###  qc-excluded-subclass-is-asserted.sparql
...
{ 
  ...
  FILTER NOT EXISTS {
    ?entity mondo:excluded_from_qc_check mondoSparqlQcMondo:qc-excluded-subclass-is-asserted.sparql .
  }
}
ORDER BY ?entity
```
...
# Tests if an excluded-subclass-of axiom is inferred
...
{ 
  ...
  FILTER NOT EXISTS {
    ?entity mondo:excluded_from_qc_check mondoSparqlQcMondo:qc-excluded-subclass-is-inferred.sparql .
  }
}
ORDER BY ?entity
```
...
SELECT DISTINCT ?entity ?xref WHERE {
    ...
    [] a owl:Axiom ;
        ...
        owl:annotatedTarget ?xref ;
        oboInOwl:source ?source .
   	    FILTER ((str(?source)="MONDO:obsoleteEquivalent") || (str(?source)="MONDO:equivalentObsolete") || (str(?source)="MONDO:obsoleteEquivalentObsolete") || (str(?source)="MONDO:equivalentTo") || (str(?source)="MONDO:relatedTo") || (str(?source)="MONDO:mondoIsNarrowerThanSource") || (str(?source)="MONDO:mondoIsBroaderThanSource") || (str(?source)="MONDO:includedEntryInOMIM") || (str(?source)="MONDO:directSiblingOf"))
    }
    ...
    FILTER (strstarts(str(?xref), "OMIM:") || strstarts(str(?xref), "OMIMPS:") || strstarts(str(?xref), "DOID:") || strstarts(str(?xref), "Orphanet:") || strstarts(str(?xref), "OMIA:"))
    FILTER (isIRI(?entity) && STRSTARTS(str(?entity), "http://purl.obolibrary.org/obo/MONDO_"))	
}
ORDER BY ?entity
```
