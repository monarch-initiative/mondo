...
parser.add_argument(
    "report_obsoletion_candidates",
    type=str,
    help="A TSV file containing the current obsoletion candidates",
)
parser.add_argument(
    "output", type=str, help="A TSV path for the output of changed terms"
)
parser.add_argument(
    "output_new", type=str, help="A TSV path for the output of new terms"
)
...
def load_report_data(fn, prefix):
    ...
    df["mondo_id"] = [
        i.replace("<http://purl.obolibrary.org/obo/MONDO_", "MONDO:").replace(">", "")
        for i in df["mondo_id"]
    ]
    df_m = pd.melt(
        df,
        id_vars="mondo_id",
        value_vars=["label", "definition", "obsoletion_candidate", "obsolete"],
    )
...
df_report_current_obsoletion_candidates = pd.read_csv(
    report_current_obsoletion_candidates, sep="\t"
)
...
df_obsoleted = df_report_current_release[
    (df_report_current_release["property"] == "obsolete")
    & (df_report_current_release["value_new"].notnull())
][["mondo_id", "value_new"]].copy()
obsoleted_terms = df_obsoleted["mondo_id"].unique()
df_labels = df_report_current_release[df_report_current_release["property"] == "label"][
    ["mondo_id", "value_new"]
].copy()
df_labels.drop_duplicates(inplace=True)
df_labels.columns = ["Mondo ID", "Label"]

df_report_full = pd.merge(
    df_report_last_release,
    df_report_current_release,
    on=["mondo_id", "property"],
    how="right",
)
df_report_full.fillna("", inplace=True)
df_report = df_report_full[
    df_report_full["value_old"] != df_report_full["value_new"]
].copy()

df_report_new = df_report[df_report["mondo_id"].isin(new_terms)].copy()
del df_report_new["value_old"]
df_report_new_wide = df_report_new.pivot(
    index="mondo_id", columns="property", values="value_new"
)
...
df_report_new[["obsolete", "obsoletion_candidate"]] = df_report_new[
    ["obsolete", "obsoletion_candidate"]
].replace(r"^\s*$", "", regex=True)
df_report_current_obsoletion_candidates[
    ["issue"]
] = df_report_current_obsoletion_candidates[["issue"]].replace(r"[<>]", "", regex=True)

df_report_new.to_csv(output_new, sep="\t", index=False)
df_report_changed.to_csv(output, sep="\t", index=False)
df_report_current_obsoletion_candidates.to_csv(
    report_current_obsoletion_candidates, sep="\t", index=False
)

print(
    "----------------START LOG: DO NOT INCLUDE ME IN RELEASE NOTES----------------------"
)
print("")
...
if not df_report_current_obsoletion_candidates["mondo_id"].is_unique:
    ...
    print(
        pd.concat(
            g
            for _, g in df_report_current_obsoletion_candidates.groupby("mondo_id")
            if len(g) > 1
        )
    )
    print("")
...
if (
    len(
        df_report_current_obsoletion_candidates[
            df_report_current_obsoletion_candidates["obsoletion_date"] == ""
        ]
    )
    > 0
):
...
if (
    len(
        df_report_current_obsoletion_candidates[
            df_report_current_obsoletion_candidates["issue"] == ""
        ]
    )
    > 0
):
...
print("")
dfx = (
    df_report_changed.groupby(["mondo_id", "property"]).size().reset_index(name="freq")
)
...
mondoidsobsoletioncandidate = df_report_current_obsoletion_candidates[
    "mondo_id"
].unique()
...
print(
    "----------------END LOG: DO NOT INCLUDE ME IN RELEASE NOTES----------------------"
)

# Preparing data frames for printing.

df_changed_labels = df_report_changed[
    (df_report_changed["property"] == "label")
    & ~(df_report_changed["mondo_id"].isin(obsoleted_terms))
]
df_changed_definitions = df_report_changed[
    (df_report_changed["property"] == "definition")
    & ~(df_report_changed["mondo_id"].isin(obsoleted_terms))
]
df_obsolete_terms = df_report_changed[(df_report_changed["property"] == "obsolete")]
df_obsolete_candidates = df_report_changed[
    (df_report_changed["property"] == "obsoletion_candidate")
    & (df_report_changed["value_old"] != True)
    & ~(df_report_changed["mondo_id"].isin(obsoleted_terms))
]
df_old_obsolete_candidates = df_report_changed[
    (df_report_changed["property"] == "obsoletion_candidate")
    & (df_report_changed["value_old"] == True)
    & ~(df_report_changed["mondo_id"].isin(obsoleted_terms))
]
...
print(
    f"* Number of terms who were previously candidate for obsoletion and are now not anymore: {ct_old_obsolete_candidates}"
)
print("")
print("")
...
print("## New terms")
df_report_new.columns = [
    "Mondo ID",
    "Label",
    "Definition",
    "Obsoletion candidate?",
    "Obsolete",
]
...
df_changed_definitions.columns = [
    "Mondo ID",
    "property",
    "Previous release",
    "New release",
...
df_obsolete_candidates.columns = [
    "Mondo ID",
    "property",
    "Previous release",
    "New release",
]
df_old_obsolete_candidates.columns = [
    "Mondo ID",
    "property",
    "Previous release",
    "New release",
...
df_changed_definitions = df_changed_definitions.merge(
    df_labels, how="left", on="Mondo ID"
)
df_obsolete_terms = df_obsolete_terms.merge(df_labels, how="left", on="Mondo ID")
df_obsolete_candidates = df_obsolete_candidates.merge(
    df_labels, how="left", on="Mondo ID"
)
df_old_obsolete_candidates = df_old_obsolete_candidates.merge(
    df_labels, how="left", on="Mondo ID"
)
