...
DEFAULTS = {
    "input_path_exclusion_reasons": os.path.join(
        PROJECT_DIR, "src", "scripts", "exclusion_reasons.csv"
    ),
...
def run(input_path_exclusion_reasons: str, input_path_mondo_schema: str):
    ...
    exclusion_dicts: Dict[str, Dict[str, str]] = mondo_schema["enums"][
        "entity_type_enum"
    ]["permissible_values"]
    ...
    obsoleted_exclusion_reasons = [
        x for x in exclusion_dicts if x not in set(df["codes"])
    ]
...
def cli():
    ...
    package_description = "Updates LinkML Mondo schema exclusion reasons enum from GoogleSheet CSV export."
    parser = ArgumentParser(description=package_description)

    parser.add_argument(
        "-e",
        "--input-path-exclusion-reasons",
        required=False,
        default=DEFAULTS["input_path_exclusion_reasons"],
        help="Path to source exclusion reasons CSV to update from.",
    )
    parser.add_argument(
        "-s",
        "--input-path-mondo-schema",
        required=False,
        default=DEFAULTS["input_path_mondo_schema"],
