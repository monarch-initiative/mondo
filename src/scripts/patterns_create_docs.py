...
def render_token(var, mapping):
    ...
    if p.match(var_str) and len(re.findall(r"'([^']*)'", var_str)) > 1:
        link_str = re.sub(
            r"'([^']*)'",
            lambda m: curie_to_link(pattern["classes"][m.group(1)]),
            var_str,
        )
        ...
        if p.match(var_str):
            link_str = re.sub(
                r"'([^']*)'",
                lambda m: curie_to_uri(pattern["classes"][m.group(1)]),
                var_str,
            )
        else:
...
def render_equivalent(text, vars, pattern):
    ...
    ret = re.sub(
        r"'([^']*)'",
        lambda m: "[" + m.group(1) + "](" + curie_to_uri(mapping[m.group(1)]) + ")",
        ret,
    )
    return ret
...
for pattern_file in pattern_files:
    ...
    with md_file_path.open("w") as fout:
        ...
        if "name" in pattern:
            fout.write("## Name \n\n")
            fout.write(
                render_str(pattern["name"]["text"], pattern["name"]["vars"], pattern)
            )
            fout.write("\n\n")
        if "annotations" in pattern:
            ...
            for anno in pattern["annotations"]:
                ...
                fout.write(
                    render_token(
                        anno["annotationProperty"], pattern["annotationProperties"]
                    )
                    + ": "
                    + render_str(anno["text"], anno["vars"], pattern)
                )
                fout.write("\n\n")
        if "def" in pattern:
            fout.write("## Definition \n\n")
            fout.write(
                render_str(pattern["def"]["text"], pattern["def"]["vars"], pattern)
            )
            fout.write("\n\n")
        if "equivalentTo" in pattern:
            ...
            fout.write(
                render_equivalent(
                    pattern["equivalentTo"]["text"],
                    pattern["equivalentTo"]["vars"],
                    pattern,
                )
            )
            fout.write("\n\n")
        ...
        if tsv_file.is_file():
            ...
            try:
                ...
                if not df.empty:
                    ...
                    fout.write(
                        re.sub(
                            r"http://purl.obolibrary.org/obo/([^_]+)_([^\s]+)",
                            lambda m: f"[{m.group(1)}:{m.group(2)}]({m.group(0)})",
                            sample_table,
                        )
                    )
                    fout.write("\n\n")
...
with index_md_path.open("w") as fout:
    ...
    for pattern_file_name, pattern in pattern_lst:
        ...
        description = re.sub(
            r"(http://purl[^\s]*\.[yaml|sparql]+)",
            lambda m: f"[{m.group(1).split('/')[-1]}]({m.group(1)})",
            description,
        )
        fout.write(
            f"| [{pattern['pattern_name']}]({pattern_file_name}/) | {description} | \n"
        )
