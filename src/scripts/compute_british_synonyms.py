...
for syn_type in df_synonyms["synonym_type"].unique():
    ...
    df_be_syns["Type"] = "http://purl.obolibrary.org/obo/OMO_0003005"
    ...
    df_be_syns.loc[-1] = [
        "ID",
        "A " + syn_type,
        ">AI http://www.geneontology.org/formats/oboInOwl#hasSynonymType",
    ]  # adding a row
    ...
    df_be_syns.to_csv(
        os.path.join(
            template_dir, "be_syns_" + syn_type[syn_type.rindex("#") + 1 :] + ".csv"
        ),
        index=False,
    )
