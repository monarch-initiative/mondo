...
for pattern_doc in pattern_docs:
    ...
    if 'annotationProperties' in pattern:
        ...
        pattern['annotationProperties'] = { 'exact_synonym': 'oio:hasExactSynonym', 'related_synonym': 'oio:hasRelatedSynonym' }

    for key in pattern:
        if isinstance(pattern[key],dict):
            if 'text' in pattern[key]:
                if re.search("[%]($|\s)",pattern[key]['text']):
                    print("Replacing % with % s potentially broken text: %s", str(pattern[key]['text']))
                    pattern[key]['text'] = re.sub("[%](?=\s|$)","%s",pattern[key]['text'])
    ...
    if 'vars' in pattern:
        for key in pattern['vars']:
            if pattern['vars'][key]=="owl:Thing":
                ...
                pattern['vars'][key]=pattern['vars'][key].replace('owl:Thing', 'owl_thing')
    ...
    if 'annotations' in pattern:
        ...
        for annotation in pattern['annotations']:
            annotation['annotationProperty'] = annotation['annotationProperty'].replace('oio:hasExactSynonym','exact_synonym')
            annotation['annotationProperty'] = annotation['annotationProperty'].replace('oio:hasRelatedSynonym','related_synonym')
    ...
    ruamel.yaml.round_trip_dump(pattern, stream=open(outfile, 'w'), indent=4, block_seq_indent=2,
