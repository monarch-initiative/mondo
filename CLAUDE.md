...
- Check first if `obo-grep.pl` is on your path. If it is not, use `perl ./obo-grep.pl` instead of `obo-grep.pl`
...
- If for some reason `obo-checkin.pl` and `obo-checkout.pl` are NOT in your PATH (and ONLY then), you can use `perl obo-checkin.pl` and `perl obo-checkout.pl` respectively.
...
1. **View the issue**: Use `gh issue view [number]` to read the issue content
